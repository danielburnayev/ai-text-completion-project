...
def prompt_user():
    ...
    settings = [0.5, 0.4]
    while True:
        response = input(f"You (to {curr_model[0]}); temp: {settings[0]}, top-p: {settings[1]}: ").strip()
        response_split = response.split()
        print("")

        if response.lower() in LEAVE_KEYWORDS:
            break
...
def help_text():
    ...
    print("Set the creativity and ideation of all AI models here to floats between 0 and 1 with:\n\t\"temperature float\" and \"top-p float\" respectively.")
...
def get_ai_response(person_response, selected_model, settings):
    try:
        ...
        print(f"AI ({selected_model[0]}); temp: {settings[0]}, top-p: {settings[1]}: ", completion.choices[0].message.content, "\n")
    except Exception:
